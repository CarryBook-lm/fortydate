...
export default function Accueil({ onDeconnexion }) {
  ...
  // Nouvelles annonces depuis la dernière visite de l'écran Annonces
  useEffect(() => {
    if (!moi?.id) return
    let annule = false
    ;(async () => {
      let vu = ''
      try { vu = localStorage.getItem('fd_annonces_vu') || '' } catch (_) {}
      let q = supabase.from('annonces').select('id', { count: 'exact', head: true }).neq('auteur_id', moi.id)
      if (vu) q = q.gt('cree_at', vu)
      const { count } = await q
      if (!annule) setNouvAnnonces(count || 0)
    })()
    return () => { annule = true }
  }, [moi?.id, overlay]) // eslint-disable-line
  ...
  const ouvrirOverlay = (v) => {
    if (v === 'annonces') {
      try { localStorage.setItem('fd_annonces_vu', new Date().toISOString()) } catch (_) {}
      setNouvAnnonces(0)
    }
    setOverlay(v); setMenuOuvert(false)
  }
  ...
  return (
    <div className="fdh-app">
      ...
      <header className="fdh-header">
        ...
        <button className="fdh-annonce-btn" onClick={() => ouvrirOverlay('annonces')} aria-label="Annonces" title="Annonces">
          📢{nouvAnnonces > 0 && <span className="fdh-pastille-or">{nouvAnnonces > 9 ? '9+' : nouvAnnonces}</span>}
        </button>
